  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      <button
        ...
        className="relative inline-flex h-12 w-12 items-center justify-center rounded-lg border border-stone-200 bg-white text-slate-700 shadow-sm transition hover:border-emerald-300 hover:bg-cream-50 hover:text-emerald-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-estate-700 focus-visible:ring-offset-2"
        onClick={() => {
      ...
      >
        <BellIcon />
        {unreadCount > 0 ? (
          <span className="absolute -right-1 -top-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-red-600 px-1 text-[11px] font-bold leading-none text-white ring-2 ring-white">
      ...
      {isOpen ? (
        ...
        >
          <div className="flex items-start justify-between gap-4 border-b border-stone-100 px-4 py-4">
            ...
            <button
              className="rounded-md px-2 py-1 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-50 hover:text-emerald-800 disabled:cursor-not-allowed disabled:text-stone-400 disabled:hover:bg-transparent"
            ...
            {notifications.map((notification) => {
              const content = (
                <div className="flex gap-3">
                  <span className="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-stone-200 bg-white text-sm font-semibold text-emerald-800">
              ...
              return (
                <article
                  className={`mx-2 rounded-lg px-3 py-3 transition ${
                    !notification.isRead ? 'bg-emerald-50/70' : 'bg-white hover:bg-cream-50'
                  }`}
                  key={notification.id}
                >
                  {notification.href ? (
